...
The VHA Information Systems Technology and Architecture (*VISTA*) is the integrated lifelong healthcare information system designed and built by VA to support the clinical, business, and financial operations of the VHA. VISTA’s database contains over  350 million veteran-years of data spanning over forty years and continues to grow at the rate of over four million new lab tests, documents, and images each day. VA has published over 35,000 peer reviewed medical studies over the past decade based on VISTA data, enabling the VA to provide the highest quality evidence-based healthcare in the United States.

VISTA's database is a VA-developed hierachical file store called VA File Manager (VA Fileman).  VA Fileman is the transactional database of VISTA managing all healthcare transactions at all VHA care sites across the United States.  Each day VA Fileman enables 350,000 staff perform over 250 million transactions at over 1250 healthcare facilities, all with millisecond latency and with 99.999996% uptime (less than one second of downtime a month). In 2023,  VA Fileman supported over 110 million veteran care encounters.  

### Problem statement

VA Fileman is indexed on a per-patient basis and optimized for the highest reliability and performance for transaction processing. VA Fileman, however, is not designed as an analytical or reporting database.  Over the years, VA has developed several mechanisms to extract subsets of VA Fileman data for secondary use and analytics, however there is no comprehensive mechanism to export, query, and manage  Fileman data.  Only a relatively small subset of Fileman's data (less than 15%) is accessible to analytical systems today, creating blind spots that affect the quality of research, trustworthiness of AI models, and accuracy of clinical decision support systems.

A comprehensive modern cloud-nat9ve approach is thus needed to provide access to all VA Fileman data for interfacing, integration, and syndication of VISTA data with VA's reporting and research systems, and to support trustworthy, high-fidelity machine learning, analytics, and clinical decision support. A modern, mainstream, commercially maintainable database for VISTA data is also necessary for VA to meet VHA data governance mandate, which requires VHA to store all veterans health data in digital form for 75 years.

### Proposal

VA Fileman contains over 6,000 files and 75,000 fields with cross-references and pointers connecting all of these files and fields in a well-defined data model. In present day terms, VA Fileman would be classified as a Document database, for which there are several commercial off-the-shelf versions available (MongoDB, DocumentDB, and others). 

Comprehensive full-fidelity replication of VA Fileman into a modern mainstream cloud-native Document database (preserving all Fileman data and nuance- (all files, fields, pointers, cross reference, and metadata)  will allow comprehensive access,  management, and query of all Veteran health data in a modern mainstream commercially-supported cloud-native database via an modern query API, making all VISTA data comprehensively available at full-fidelity to all VA analytical and AI systems. (Figure 1).
...
Criteria | VistA database <br>(VA FileMan) |  Cloud-native database <br> (MongoDB)
--- | --- | ---

...
VISTA's database is a VA-developed hierachical file store called VA File Manager (VA Fileman).  VA Fileman is the transactional database of VISTA managing all healthcare transactions at all VHA care sites across the United States.  Each day VA Fileman enables 350,000 staff perform over 200 million transactions with sub-second latency with 99.999996% uptime (less than one second of downtime a month). In 2023,  VA Fileman supported over 110 million veteran care encounters.  
...
VA Fileman is indexed on a per-patient basis and optimized for the highest reliability and performance for operations at all VHA care facilities. VA Fileman, however, is not designed as an analytical or reporting database.  Over the years, VA has developed several mechanisms to extract subsets of VA Fileman data for secondary use and analytics, however there is no comprehensive mechanism to export, query, and manage  Fileman data.  Only a relatively small subset of Fileman's data (less than 15%) is accessible to analytical systems, creating blind spots that affect the quality of research, trustworthiness of AI models, and accuracy of clinical decision support systems.

A comprehensive approach is thus needed to provide full-fidelity access to VA Fileman data for interfacing, integration, and syndication of VISTA data with VA's reporting and research systems, and to support trustworthy, full-fidelity machine learning, analytics, and clinical decision support. A modern, mainstream, commercially maintainable database for VISTA data is also necessary for VA to meet VHA data governance mandate, which requires VHA to store all veterans health data in digital form for 75 years.
